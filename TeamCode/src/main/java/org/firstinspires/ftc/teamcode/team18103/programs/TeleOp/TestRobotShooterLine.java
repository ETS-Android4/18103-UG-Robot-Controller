...
@TeleOp
public class TestRobotShooterLine extends OpMode {
    ...
    public void loop() {
        ...
        if(gamepad1.y) {
            robot.getIOSubsystem().outtakeFromPoint3(Math.hypot(60, 0));
        } else {
            robot.getIOSubsystem().runOuttake(false);
        }

        if(gamepad1.a) {
            robot.getIOSubsystem().outtakeFromPoint3(Math.hypot(4*24, 0));
        } else {
            robot.getIOSubsystem().runOuttake(false);
        }

        if(gamepad1.b) {
            robot.getIOSubsystem().outtakeFromPoint3(Math.hypot(120, 0));
        } else {
            robot.getIOSubsystem().runOuttake(false);
        }
